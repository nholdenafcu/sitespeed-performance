...
set DOCKER_SETUP=--cap-add=NET_ADMIN  --shm-size=2g --rm -v /config:/config -v "$(pwd):/sitespeed.io" -v /etc/localtime:/etc/localtime:ro -e MAX_OLD_SPACE_SIZE=3072
...
for %%f in (%~dp0tests\*.txt %~dp0tests\*.js) do (
    for %%a in (%DESKTOP_BROWSERS_DOCKER%) do (
        ...
        @REM docker run %DOCKER_SETUP% %DOCKER_CONTAINER% --config %CONFIG_FILE% -b %%a %%f
    )
)
